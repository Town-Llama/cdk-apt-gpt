...
function buildJsonPromptData(data, address) {
    let prompt = '';
    const columnMapping = {
      price: 'Monthly Rent',
    ...
    prompt += '{';
    const elements = Object.keys(columnMapping).map(columnName => {
      const value = getColumnValue(data, columnName);
      if (value !== undefined) {
        const description = columnMapping[columnName] || columnName;
