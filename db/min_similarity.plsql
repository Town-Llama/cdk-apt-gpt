CREATE OR REPLACE FUNCTION search_properties_with_embeddings_min(
    min_rent NUMERIC,
    max_rent NUMERIC,
    bedrooms INTEGER,
    input_lat NUMERIC,
...
BEGIN
    ...
    SELECT fu.*,
           (MIN((e.data <-> embedding_vector)::DECIMAL(9, 6))+AVG((e.data <-> embedding_vector)::DECIMAL(9, 6)))/2 AS embedding_similarity
